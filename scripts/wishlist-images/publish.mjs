...
import { loadEnv, loadItems, setImageUrl } from "./db.mjs";
...
async function rollback(file, confirm) {
  ...
  for (const entry of entries) {
    ...
    if (confirm) await setImageUrl(entry.id, entry.previous);
  }
...
async function main() {
  ...
  const plan = selection.map(({ id, file }) => {
  ...
  await writeFile(rollbackFile, `${JSON.stringify(plan.map(({ id, previous, next }) => ({ id, previous, next })), null, 2)}\n`);
