...
export class CityMarkerManager {
  ...
  createMarkersFromCities(
    ...
    Logger.debug(`Created ${visitedCount} city markers`);
  ...
  private createSingleMarker(
    ...
    labelElement.setHTMLUnsafe(
      `<div class="city-label-flag"><div class="fi fi-${city.properties.code} flag-fix"></div></div><div class="city-label-overlay-content">${city.properties.name}, ${city.properties.country}</div>`,
    );
  ...
  private setupMarkerInteractions(
    ...
    // Optimized mobile touch/click handler
    const handleMarkerInteraction: MarkerInteractionHandler = (e: Event) => {
      e.stopPropagation();
      e.preventDefault();
      this.preventMapClick();

      // Use debouncer to prevent rapid interactions
      debouncer.debounceFrame("markerInteraction", () => {
  ...
  private toggleMarkerLabel(
    ...
    // Batch visibility check and operations
    domBatcher.read(() => {
   ...
   */
  showMarkers(): void {
    PerfTimer.start("showMarkers");

    // Batch marker addition for better performance
    if (this.map) {
      domBatcher.write(() => {
   ...
   */
  hideMarkers(): void {
    PerfTimer.start("hideMarkers");

    // Batch marker removal
    domBatcher.write(() => {
      this.markers.forEach((marker) => {
        marker.remove();
        performanceMonitor.recordDOMOperation();
  ...
  addLabelsToContainer(): void {
    PerfTimer.start("addLabelsToContainer");
    ...
    PerfTimer.end("addLabelsToContainer");
  }
}
