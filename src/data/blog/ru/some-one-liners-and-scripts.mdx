...
description: "Команды, к которым возвращаюсь: переписать автора коммита, добавить только отслеживаемые файлы, убить процесс на порту."
...
Нужно освободить порт, который занял какой-то процесс? Этот однострочник найдёт и убьёт его:

```bash
lsof -ti :PORT | xargs kill -9
```

Например, чтобы завершить всё, что работает на порту 3000:

```bash
lsof -ti :3000 | xargs kill -9
```

### Не давать маку уснуть

`caffeinate` уже есть в macOS. Ни приложения в меню-баре, ни `brew install`.

Без флагов держит систему от idle sleep, пока не нажмёшь Ctrl-C:
...
```bash
caffeinate -d   # экран не гаснет
caffeinate -i   # нет idle sleep (это дефолт)
caffeinate -m   # диск не засыпает
caffeinate -s   # не спать вообще — только от сети
caffeinate -u   # сделать вид, что юзер что-то нажал, разбудит экран
```

Ограничить можно двумя способами. Таймаут в секундах:

```bash
caffeinate -i -t 3600
```

Или передать команду — запрет на сон живёт ровно столько, сколько живёт процесс:
...
Вторая форма — та, которую стоит запомнить. Ничего не надо выключать потом. `-w PID` делает то же самое для уже запущенного процесса.

Зачем это мне: оставить Claude Code сессии работать на ноуте дома, пока меня нет, и подключаться к ним с телефона через Remote Control. Спящий ноут ничего не доделает и уронит соединение.

```bash
caffeinate -is
```

**Держи его в розетке.** `-s` работает только от сети, а с закрытой крышкой на батарее ноут уснёт в любом случае.
