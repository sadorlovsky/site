...
description: "Мысленная модель и команды, которые держу под рукой: где что живёт в Claude Code и в чём подвох."
...
{/* 📊 PLACEHOLDER: диаграмма — горизонтальные полосы сравнивающие расход контекста при разных MCP конфигах. Строка 1: нет MCP (baseline). Строка 2: только GitHub MCP (~46k токенов). Строка 3: GitHub + Slack + Sentry (~55k токенов). Обозначить сколько свободного места остаётся в каждой строке. Суть — сделать цифры ощутимыми. */}
...
{/* 📱 PLACEHOLDER: скриншот — экран телефона с открытым claude.ai/code и активной Claude Code сессией. Желательно с реальным проектом а не демо. Должно выглядеть как реальное использование с дивана, не продуктовый шот. */}
