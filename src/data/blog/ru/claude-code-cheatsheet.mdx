...
> Remote Control (фев 2026, preview): запускаешь в CLI, получаешь ссылку/QR, открываешь на телефоне — разговор синхронизируется, но Claude Code всё равно работает на твоей машине. Терминал должен быть открыт.
...
- Требует подписку claude.ai (Pro/Max/Team/Enterprise) — API key не работает
...
**Ограничения:** нет доступа к локальным файлам и MCP серверам, свежее окружение каждый раз, работает только с GitHub репо.
