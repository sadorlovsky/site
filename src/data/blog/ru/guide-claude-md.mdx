...
Auto memory — дополнение. Claude сам записывает заметки по ходу работы (команды билда, инсайты отладки, твои предпочтения). Загружаются первые 200 строк `MEMORY.md` при старте сессии. Подробнее — в секции [Auto memory](#auto-memory).
