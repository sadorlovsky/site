...
Первый месяц использования Claude Code я вручную вводил одну и ту же последовательность деплоя каждый раз. Те же шаги, те же команды, те же мысли. Однажды я прикинул: я написал "запусти bun test, потом build, потом проверь preview URL" примерно сорок раз в разных сессиях. Вот тогда skills стали очевидными.
...
Больше всего времени я убил на skill который не триггерился автоматически. Минут 15 проверял путь, имя, синтаксис frontmatter. Оказалось, в `description` было "creates a summary", а я каждый раз спрашивал "write a session log". Эти две фразы не пересекаются — Claude так и не загружал skill. Переписал два предложения, всё заработало.
