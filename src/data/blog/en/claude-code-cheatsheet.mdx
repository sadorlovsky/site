---
title: 'My Claude Code Cheat Sheet'
pubDate: 2026-04-02
...
**In short:** Chat is claude.ai in a native window. Code is the Claude Code CLI, but with a nice UI instead of a terminal.
...
> Remote Control (Feb 2026, preview): start it in CLI, get a link/QR, open on your phone — conversation syncs, but Claude Code still runs on your machine. Terminal must stay open.

---

## 3. MCP and Context — The Token Truth

**Yes, MCP always eats context** — this is not a myth.
...
**Why are people "leaving MCP"?** They're not, but there are real problems:
...
- Requires claude.ai subscription (Pro/Max/Team/Enterprise) — API key doesn't work
...
**Limitations:** no access to local files and MCP servers, fresh environment each time, only works with GitHub repos.
