...
description: "The mental model and command snippets I keep open while using Claude Code: what lives on which surface, and where the catches are."
...
{/* 📊 PLACEHOLDER: diagram — horizontal bars comparing context consumed by MCP setups. Row 1: no MCP (baseline). Row 2: GitHub MCP only (~46k tokens). Row 3: GitHub + Slack + Sentry (~55k tokens). Label the free space left in each row. Point is to make the numbers visceral. */}
...
{/* 📱 PLACEHOLDER: screenshot — phone screen showing claude.ai/code with an active Claude Code session. Ideally with a real project visible, not a demo. Should look like someone actually using this from their couch, not a product shot. */}
