...
Every CLAUDE.md I've ever written has gone through the same arc: start comprehensive, watch Claude ignore half of it, strip it down to what actually matters, feel annoyed I wrote the rest. The current one on this project is 40 lines. The first version was 200.

{/* 😤 PLACEHOLDER: meme — something about writing a detailed guide that gets ignored. "Instructions: do X carefully. Claude: *does something completely different*" or the "nobody asked" format. Should be relatable to anyone who's written docs for an AI. */}

## What is CLAUDE.md

Most people treat `CLAUDE.md` like a note file — dump everything in there, hope for the best. That usually ends in disappointment: too vague, too long, too much generic advice, and the model ignores half of it.
...
Longer usually means two problems at once: worse adherence and more context spent on the file itself. If it keeps growing, split it into `.claude/rules/` or use imports.
...
I keep auto memory mostly off for shared projects and on for personal ones. The notes Claude writes tend to be useful for solo work — build quirks, preferences it inferred, things you mentioned once. In shared setups it can get weird: memory files accumulate noise from different contexts and you end up with a "memory" that's half-outdated.
