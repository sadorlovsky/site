...
Reach for this when context is filling up but you're not done yet — `/compact` keeps you in the session without losing the thread.
...
I run mine at 60%. The default 95% feels like waiting until you're out of gas to start looking for a station.
...
**"Kitchen sink" session.** Started with one thing, then asked something unrelated, then came back. Context full of junk. Fix: `/clear` between unrelated tasks.

This is the one I fall into most often. The session starts as "fix the login bug" and two hours later I'm asking about CSS in a session that's read 40 TypeScript files. The bug is fixed but Claude now has opinions about both the auth layer and the button spacing.

**Endless corrections.** Claude does it wrong, you correct, still wrong. Fix: after two failed corrections, stop — the context is polluted with broken approaches. `/clear` and write a better initial prompt using what you've learned.

**Unbounded exploration.** "Investigate X" with no scope → Claude reads hundreds of files. Fix: narrow it, or send a subagent.

**Trust but don't verify.** Claude produced something that looks right, you shipped it.

{/* 🐕 PLACEHOLDER: meme — "This is fine" (dog in burning room). Caption: "me, after letting Claude write an auth middleware I didn't test" */}

Fix: always give Claude a way to check its own work — tests, a script, a screenshot, anything.
