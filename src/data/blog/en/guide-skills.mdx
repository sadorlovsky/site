...
For the first month I used Claude Code, I typed the same deploy sequence manually every single time. Same steps, same commands, same mental overhead. One day I counted: I’d typed “run bun test, then build, then check the preview URL” about forty times across different sessions. That’s the moment skills clicked for me.
...
The most time I've wasted on this: a skill that wouldn't auto-trigger. Fifteen minutes checking paths, names, frontmatter syntax. Turned out the `description` said "creates a summary" and I kept asking Claude to "write a session log". Those two phrases have zero overlap — Claude never loaded it. Rewrote two sentences, fixed immediately.
